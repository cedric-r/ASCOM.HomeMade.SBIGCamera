//
// ASCOM.HomeMade.SBIGHub Local COM Server
//
...
namespace ASCOM.HomeMade.SBIGHub
{
    public static class Server
    {
        ...
        private static string s_appId = "{9252e3bb-4996-421a-b3d0-5396a5d78996}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "ASCOM.HomeMade.SBIGHub", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The ASCOM.HomeMade.SBIGHub was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "ASCOM.HomeMade.SBIGHub", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "ASCOM.HomeMade.SBIGHub", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "ASCOM.HomeMade.SBIGHub", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
